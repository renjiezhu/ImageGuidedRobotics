ImageGuidedRobotics by arcLAB at UCSD
================================

Customized version of Slicer for image guided robotics for lung biopsy

![ImageGuidedRobotics by arcLAB at UCSD](Applications/App/Resources/Images/LogoFull.png?raw=true)
...
* Setting up your git account:
    ...
    * If not already done, email `Jean-Christophe Fillion-Robin <jchris.fillionr@kitware.com>` to be granted access to
    the [renjiezhu/ImageGuidedRobotics](https://github.com/renjiezhu/ImageGuidedRobotics) repository.
...
mkdir W
cd /c/W
git clone https://github.com/renjiezhu/ImageGuidedRobotics.git ImageGuidedRobotics
```
...
1. Start [CMake GUI](https://cmake.org/runningcmake/), select source directory `C:\W\ImageGuidedRobotics` and set build directory to `C:\W\ImageGuidedRobotics-rel`.
2. Add an entry `QT_QMAKE_EXECUTABLE` pointing to `C:\D\Support\qt-4.8.7-64-vs2013-rel`.
2. Generate the project.
3. Open `C:\W\ImageGuidedRobotics-rel\ImageGuidedRobotics.sln`, select `Release` and build the project.

<b>Option 2: Command Line</b>

1. Start the [Command Line Prompt](http://windows.microsoft.com/en-us/windows/command-prompt-faq)
2. Configure and build the project in `C:\W\ImageGuidedRobotics-rel` by typing the following commands:

```bat
cd C:\W\
mkdir ImageGuidedRobotics-rel
cd ImageGuidedRobotics-rel
cmake -G "Visual Studio 14 2015 Win64" -DQT_QMAKE_EXECUTABLE:PATH=C:\Qt\5.9.1\msvc2015_64\bin\qmake.exe ..\ImageGuidedRobotics
...
1. In the `C:\W\ImageGuidedRobotics-rel\Slicer-build` directory, open `Slicer.sln` and build the `PACKAGE` target
...
```bat
cd C:\W\ImageGuidedRobotics-rel\Slicer-build
